...
export async function loadAgentCaseLibrary() {
  ...
  try {
    ...
    const rawCases = Array.isArray(j?.cases) ? j.cases : [];
    libraryCache = {
      version: j.version,
      dimensions: normalizeDimensionList([
        ...(Array.isArray(j.dimensions) ? j.dimensions : []),
        ...rawCases.flatMap((c) => [
          c?.primary_dimension,
          ...(Array.isArray(c?.secondary_dimensions) ? c.secondary_dimensions : []),
        ]),
      ]),
      cases: rawCases.map((c) => ({
        ...
        primary_dimension: normalizeDimensionKey(c.primary_dimension || "concept"),
        secondary_dimensions: normalizeDimensionList(
          Array.isArray(c.secondary_dimensions) ? c.secondary_dimensions.map(String) : []
        ),
...
function relevanceScore(caseItem, row) {
  ...
  if (pri === "site_perception" && (S > 0.22 || E > 0.45)) s += 0.16;
  if (pri === "form" && (N_UD(row) > 0.55 || AC < 0.45)) s += 0.14;
  if (pri === "space" && (N_OS > 0.45 || N09 > 0.5)) s += 0.16;
  if (pri === "function" && E > 0.4) s += 0.14;
  if (pri === "technology" && (S > 0.28 || AC < 0.5)) s += 0.14;
...
function N_UD(row) {
  return Number(row.N_UD) || 0;
}

/**
 * 每维取分最高的一条（主维度匹配），保证多维拼接素材齐全。
...
export async function buildCaseLibraryPack(selectedRow) {
  ...
  const dims = lib.dimensions.length ? lib.dimensions : DEFAULT_CASE_DIMENSIONS;
  ...
  for (const d of dims) {
    ...
    const label = getDimensionLabel(d);
    if (!c) {
...
export function buildSchemeImagePrompt(pack, discussionSnippet) {
  ...
  return `建筑插画风格，俯视略透视，上海衡复历史街区街道更新概念拼贴：多维分区展示 ${titles}。柔和色块与简洁线稿，无文字无水印。设计说明氛围：${ctx}`;
}

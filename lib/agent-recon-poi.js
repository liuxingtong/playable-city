 ...
 * 配置：window.AgentReconPoiConfig；未设 proxyBase 时优先用 AgentReconCasebaseConfig.eventsApiBase，再回退 ragProxyUrl（网页检索端口无 /v1/poi-around）。
 */
...
export function getPoiRuntimeConfig() {
  ...
  const cb = window.AgentReconCasebaseConfig || {};
  const fromEvents = String(cb.eventsApiBase || "").replace(/\/$/, "");
  const fromRag = String(cb.ragProxyUrl || "").replace(/\/$/, "");
  const proxyBase = String(poi.proxyBase || fromEvents || fromRag).replace(/\/$/, "");
  return {

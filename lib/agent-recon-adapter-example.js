...
(function attachAgentReconAdapters() {
  ...
  window.AgentReconAdapters = {
    async scoreWithVisionLLM(payload) {
      ...
      try {
        // 内置 GLM 会用 precomputedImageDataUrl 省重复下载；外接 HTTP 接口请勿原样转发该字段（体积过大）
        const { precomputedImageDataUrl: _omit, ...body } = payload;
        void _omit;
        const out = await postJson("/api/agent/vision-score", body);

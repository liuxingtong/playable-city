...
const COMMENT_TASK_GUIDE =
  ...
  "**表述要求**：正文用通顺中文描述含义，**不要**单独抛出 E、N09 等字母串；若必须指指标，请写「中文含义（代号）」例如「街道舒适度 (N09) 偏低」「文化场所密度 (N_CD)」。" +
...
const DISCUSSION_AGENT_OUTPUT_GUIDE =
  ...
  "结合场地指标说明为何组合成立：请用中文表述指标含义，代号仅在括号内标注（如 开敞空间嵌入度 (N_OS)、文化场所密度 (N_CD)）。\n" +
    ...
    async scoreWithVisionLLM(payload) {
      try {
        ...
        const sys =
          ...
          "其中 reason 用通顺中文：先简述街景里看到了什么、对行人与停留意味着什么，再衔接指标判断；**禁止**在 reason 里单独堆砌 E、N09 等代号，若写指标须用「中文含义（代号）」格式。";
        const userContent = [
          {
            type: "text",
    ...
    async getAgentComment(payload) {
      try {
        ...
        if (llmScore && Number.isFinite(llmScore.score)) {
          user +=
            `\n【街景辅助得分（已纳入踏勘综合分，与规则分位加权）】数值=${llmScore.score.toFixed(3)}。` +
    ...
    async runDeepDiscussion(payload) {
      try {
        ...
        const facilitatorSys = casebaseOn
          ? `你是多 Agent 协作主持人（城市规划 + 参与式设计）。五位 Agent 已各自完成案例学习，并各提出了一个独立的设计草案。你的任务是：在这五个草案之间找出共识与分歧，从中综合出一个更完整的整合方案——而非另起炉灶或重新罗列案例。必须明确指出你吸取了哪位 Agent 草案中的哪个核心判断，以及为何舍弃或调整了其他部分。输出结构：1）五份草案的核心共识（1～3 条跨草案反复出现的判断）2）主要分歧与取舍理由 3）「整合方案」：在五份草案的基础上命名一个新方案，说明它如何融合各方核心策略（写清来源草案），以及与场地条件的关系——说明时用中文表述指标含义，代号仅在括号内标注（如 暴露度 (E)、文化场所密度 (N_CD)、开敞空间嵌入度 (N_OS)、街道舒适度 (N09) 等）4）短中长期行动建议 5）现场核验清单。${FACILITATOR_SPATIAL_SECTION} 全文总字数 900～1800 字。`
          : `你是多 Agent 协作主持人（城市规划 + 参与式设计）。五位 Agent 已各自完成案例学习，并各提出了一个独立的设计草案。你的任务是：在这五个草案之间找出共识与分歧，从中综合出一个更完整的整合方案——而非另起炉灶。必须明确指出你吸取了哪位 Agent 草案中的哪个核心判断，以及为何舍弃或调整了其他部分。输出结构：1）核心共识 2）主要分歧与取舍 3）「整合方案」（来自五份草案的融合，写清来源与策略组合逻辑；指标用中文表述，代号仅放括号）4）短中长期行动 5）现场核验清单。${FACILITATOR_SPATIAL_SECTION} 全文总字数 800～1600 字。`;

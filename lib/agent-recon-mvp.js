...
async function init() {
  ...
  if (cbCfg?.enabled) {
    ...
    casebaseHighlighter = new CaseRetrievalHighlighter({
      ...
      eventsApiBase: cbCfg.eventsApiBase || "http://127.0.0.1:8787",
    ...
    const ragBtn = document.getElementById("casebaseRagBtn");
    const ragQ = document.getElementById("casebaseQuery");
    const ragOut = document.getElementById("casebaseRagOut");
    ragBtn?.addEventListener("click", async () => {
      const q = ragQ?.value?.trim();
      if (!q || !cbCfg.ragProxyUrl) {
        if (ragOut) ragOut.textContent = "请填写检索问题，并在 AgentReconCasebaseConfig 中配置 ragProxyUrl。";
        return;
      }
      const traceId = generateTraceId();
      lastRagTraceId = traceId;
      ragBtn.disabled = true;
      if (ragOut) ragOut.textContent = "检索中（trace_id 已生成）…";
      try {
        const j = await callCasebaseRagProxy({
          ragProxyUrl: cbCfg.ragProxyUrl,
          query: q,
          agentId: (cbCfg.agentIdPrefix || "playable_") + personaSelect.value,
          traceId,
          k: cbCfg.ragK ?? 6,
        });
        if (ragOut) {
          ragOut.textContent = JSON.stringify(
            {
              compose_id: j.compose_id,
              event_id: j.event_id,
              trace_id: j.trace_id ?? traceId,
              refs: j.refs,
              answer: j.answer ?? j.response ?? j,
            },
            null,
            2
          );
        }
        const eid = Number(j.event_id);
        if (Number.isFinite(eid) && casebaseHighlighter) {
          casebaseHighlighter.sinceEventId =
            casebaseHighlighter.sinceEventId == null ? eid : Math.max(casebaseHighlighter.sinceEventId, eid);
        }
      } catch (e) {
        if (ragOut) ragOut.textContent = `错误: ${e.message || e}`;
      } finally {

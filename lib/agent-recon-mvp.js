...
/** 轨迹 OD 网综合指标（介数/调和/PageRank 合成）在 Wp 中的权重；其余七维按 (1-此值) 缩放 */
const OD_DIM_WEIGHT = 0.08;

function scalePersonaWeights7(w7, odW) {
  const f = 1 - odW;
  return [...w7.map((x) => x * f), odW];
}

const PERSONA_WEIGHTS = {
  elder: scalePersonaWeights7([0.08, 0.18, 0.22, 0.05, 0.22, 0.05, 0.2], OD_DIM_WEIGHT),
  student: scalePersonaWeights7([0.32, 0.05, 0.1, 0.22, 0.08, 0.23, 0.0], OD_DIM_WEIGHT),
  white: scalePersonaWeights7([0.08, 0.2, 0.08, 0.32, 0.32, 0.0, 0.0], OD_DIM_WEIGHT),
  family: scalePersonaWeights7([0.12, 0.28, 0.25, 0.05, 0.3, 0.0, 0.0], OD_DIM_WEIGHT),
  wander: scalePersonaWeights7([0.38, 0.05, 0.08, 0.08, 0.09, 0.32, 0.0], OD_DIM_WEIGHT),
};
...
const DEFAULT_STUDY_BOUNDARY_URL = "../data/tianping-road-street.geojson";
/** 地块轨迹 OD 指标（可选）：由 `npm run build:block-od` 生成 */
const BLOCK_OD_CSV_URL = "../data/block-od-activity.csv";
/** 边段与最近地块质心超过此距离（米）则 N_OD 取中性 0.5 */
const BLOCK_OD_MAX_MATCH_M = 900;
const CLD_CSV_URL = "../data/cld_priority.csv";

/**
 * 勘探边界 GeoJSON：`window.AgentReconStudyConfig.boundaryUrl`，或 URL 参数 `?boundary=相对路径`；
 * 默认 `data/tianping-road-street.geojson`。
 */
function getStudyBoundaryConfig() {
  const cfg = typeof window !== "undefined" && window.AgentReconStudyConfig ? window.AgentReconStudyConfig : {};
  let url = cfg.boundaryUrl != null && String(cfg.boundaryUrl).trim() ? String(cfg.boundaryUrl).trim() : "";
  if (!url && typeof window !== "undefined") {
    try {
      const q = new URLSearchParams(window.location.search).get("boundary");
      if (q && String(q).trim()) url = String(q).trim();
    } catch {
      /* ignore */
    }
  }
  if (!url) url = DEFAULT_STUDY_BOUNDARY_URL;
  const label =
    cfg.label != null && String(cfg.label).trim()
      ? String(cfg.label).trim()
      : url.includes("tianping-road-street")
        ? "上海市徐汇区天平路街道"
        : "";
  return { boundaryUrl: url, label };
}
...
async function init() {
  ...
  const studyCfg = getStudyBoundaryConfig();
  studyGeometry = await loadStudyBoundaryGeometry(studyCfg.boundaryUrl);
  if (studyGeometry) {
    addStudyBoundaryToMap(studyGeometry, studyCfg.label);
    if (studyScopeNote) {
      studyScopeNote.textContent = studyCfg.label
        ? `勘探范围：${studyCfg.label}（边界已加载，仅显示界内边段与街景索引）。`
        : `勘探范围：自定义边界 ${studyCfg.boundaryUrl.split("/").pop() || ""}（已加载）。`;
    }
  } else {
    console.warn(
      "[Agent踏勘] 未加载勘探边界 GeoJSON，退回全表。可设 boundaryUrl 或 ?boundary=…；默认天平路可运行: node scripts/fetch-tianping-road-street.mjs"
    );
    if (studyScopeNote) {
      studyScopeNote.textContent =
        "勘探范围：边界文件未加载或解析失败，当前为全表。请检查 boundaryUrl / ?boundary= 或默认 data/tianping-road-street.geojson。";
    }
  }

  const blockOdBlocks = await loadBlockOdBlocks();
  if (!blockOdBlocks.length) {
    console.warn(
      "[Agent踏勘] 未加载 data/block-od-activity.csv，N_OD 将固定为 0.5。可运行: npm run build:block-od"
    );
  }

  const rows = await d3.csv(CLD_CSV_URL);
  allRows = preprocess(rows, studyGeometry, blockOdBlocks);
...
function preprocess(rows, boundaryGeom, blockOdBlocks) {
  let parsed = rows
    .map((r) => {
      const row = {
        lon: toNum(r.lon),
  ...
  const blocks = blockOdBlocks || [];
  parsed.forEach((row) => attachNearestBlockOd(row, blocks));

  const tuples = parsed.map(rawTuple);
  const ext = featureExtent(tuples);
...
async function selectPoint(row, marker) {
  ...
  detailBox.innerHTML = `
    ...
    <div class="kv"><span class="k">N_OD（轨迹网）</span><span class="v">${fmt(row.N_OD)}</span></div>
...
function getReconStudySlug() {
  if (studyGeometry) {
    const { boundaryUrl } = getStudyBoundaryConfig();
    const name = String(boundaryUrl).split("/").pop() || "bounded";
...
function rawTuple(r) {
  ...
  const nOd = Number.isFinite(Number(r.N_OD)) ? clamp(Number(r.N_OD), 0, 1) : 0.5;
  return [toNum(r.cpvi_E), sInv, toNum(r.cpvi_AC), toNum(r.N_UD), toNum(r.N09), toNum(r.N_CD), toNum(r.N_OS), nOd];
}

function featureExtent(tuples) {
  const dim = Math.max(1, tuples[0]?.length || 8);
  const lo = Array(dim).fill(Infinity);
  const hi = Array(dim).fill(-Infinity);
  tuples.forEach((t) => {
    for (let i = 0; i < dim; i++) {
...
async function loadBlockOdBlocks() {
  try {
    const rows = await d3.csv(BLOCK_OD_CSV_URL);
    const out = [];
    for (const r of rows) {
      const lon = toNum(r.lon);
      const lat = toNum(r.lat);
      const od = toNum(r.od_composite);
      if (!Number.isFinite(lon) || !Number.isFinite(lat) || !Number.isFinite(od)) continue;
      out.push({ id: r.Id, lon, lat, od });
    }
    return out;
  } catch {
    return [];
...
function attachNearestBlockOd(row, blocks) {
  if (!blocks.length) {
    row.N_OD = 0.5;
    return;
  }
  ...
  for (const b of blocks) {
    const d = haversineM(row.lat, row.lon, b.lat, b.lon);
    if (d < bestD) {
      bestD = d;
      bestOd = b.od;
    }
  }
  row.N_OD = bestD <= BLOCK_OD_MAX_MATCH_M ? bestOd : 0.5;
}
...
function makeRuleComment(payload) {
  ...
  const poiHint =
    typeof poiContext === "string" && poiContext.trim()
      ...
      : "";
  const childPov =
    agent.id === "family" ? " 孩子视角（约 1～1.2 m）可蹲低快速扫一眼路缘与栏杆。" : "";
  const text = `${level}。建议结合街景/现场核对停留条件与动线断点。${childPov}${imageHint}${llmHint}${poiHint}`;
...
async function loadStudyBoundaryGeometry(boundaryUrl) {
  try {
    const j = await d3.json(boundaryUrl);
    let geom = null;
...
function addStudyBoundaryToMap(geometry, labelText) {
  boundaryLayer.clearLayers();
  const name = labelText && String(labelText).trim() ? String(labelText).trim() : "勘探范围";
  const tip = labelText && String(labelText).trim() ? `勘探范围：${labelText}` : "勘探范围（自定义边界）";
  const gj = L.geoJSON(
    { type: "Feature", geometry, properties: { name } },
    {
      ...
      onEachFeature(_f, layer) {
        layer.bindTooltip(tip, { sticky: true, direction: "center" });
      },
    }
  );
